...
class GloveDPCNN(GloveCNN):
    ...
    def _build_model(self, maxlen, max_features, embedding_size, embedding_matrix, repeat_nr):
        """
        Implementation of http://ai.tencent.com/ailab/media/publications/ACL3-Brady.pdf
        """

        def _base_layer(x):
            x = Conv1D(embedding_size, kernel_size=3, padding='same', activation='relu',
...
class GloveExp(GloveCNN):   
    def _build_model(self, maxlen, max_features, embedding_size, embedding_matrix):
        ...
        x = Conv1D(250, kernel_size=3, activation='relu')(x)
        x = Conv1D(250, kernel_size=3, activation='relu')(x)
        x = GlobalMaxPool1D()(x)
#         x = Dropout(0.5)(x)
        predictions = Dense(6, activation="sigmoid")(x)
